...
class DagHashManager(Contract):
    def __init__(self, node_connection: Connection, contract_address: str, contract_abi: Dict):
        super().__init__(
    ...
    def add_hash(self, value: str, private_key: str) -> BlockchainResponse | BlockchainError:
    ...
    def read_hash(self, hashed_value: str) -> BlockchainValue | BlockchainError:
    ...
    def deprecate_hash(self, hashed_value: str, private_key: str) -> BlockchainResponse | BlockchainError:
        hashed_value_bytes = Web3.to_bytes(hexstr=hashed_value)
        contract_function = self.contract.functions.deprecateHash(hashed_value_bytes)
        return self.execute(contract_function, private_key)
    
    def delete_hash(self, hashed_value: str, private_key: str) -> BlockchainResponse | BlockchainError:
        hashed_value_bytes = Web3.to_bytes(hexstr=hashed_value)
        contract_function = self.contract.functions.deleteHash(hashed_value_bytes)
        return self.execute(contract_function, private_key)
    
    def add_outgoing_link(self, from_hash: str, to_hash: str, private_key: str) -> BlockchainResponse | BlockchainError:
    ...
    def read_outgoing_links(self, hashed_value: str) -> BlockchainValue | BlockchainError:
        hashed_value_bytes = Web3.to_bytes(hexstr=hashed_value)
        contract_function = self.contract.functions.readOutgoingLinks(hashed_value_bytes)
        return self.call(contract_function)
    
    def delete_outgoing_link(self, from_hash: str, to_hash: str, private_key: str) -> BlockchainResponse | BlockchainError:
        from_hash_bytes = Web3.to_bytes(hexstr=from_hash)
        to_hash_bytes = Web3.to_bytes(hexstr=to_hash)
